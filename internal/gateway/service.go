...
type Service interface {
	...
	ListAllModels(ctx context.Context, filter ModelFilter) ([]api.Model, error)
...
func (s *service) RegisterProvider(ctx context.Context, p llm.Provider) error {
	...
	s.logger.Info("provider registered", zap.String("name", p.Name()), zap.Int("models", len(models)))
	return nil
}

func (s *service) Chat(ctx context.Context, req *server.ChatRequest) (*server.ChatResponse, error) {
	provider, upstreamModelID, err := s.resolveProvider(req.Model)
	if err != nil {
		return nil, err
	}
...
func (s *service) GetProviderForModel(ctx context.Context, modelID string) (llm.Provider, string, error) {
	...
	if err != nil {
		return nil, "", server.BadRequestError(fmt.Sprintf("route resolution failed for model '%s': %v", modelID, err))
	}
...
func (s *service) GetProvider(providerID string) (server.Provider, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *service) StreamChat(ctx context.Context, req *server.ChatRequest) (<-chan server.StreamResult, error) {
