...
func TestWebSearchExtension_ExecuteConcatenatedArguments(t *testing.T) {
	mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		resp := searxngResponse{
			...
			Results: []struct {
				...
				{URL: "https://example.com/final", Title: "Final", Content: "Used sanitized args"},
			},
		}
		w.Header().Set("Content-Type", "application/json")
	...
	ext := NewWebSearchExtension(mockServer.URL)
	// Simulate a buggy OpenAI-compatible provider that concatenates full
	// argument snapshots into one string.
	args := []byte(`{"query":"first"}{"query":"second","max_results":1}`)
